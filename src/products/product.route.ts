...
return res.status(StatusCodes.BAD_REQUEST).json({error: 'Please fill all fields' });
}
...
return res.status(StatusCodes.OK).json({message: `Product with ID ${req.params.id} deleted`});
} catch(error){
return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({error});
}
})

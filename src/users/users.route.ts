...
userRouter.get("/users", async (req: Request, res: Response) => {
    try {
        ...
        if (!allUsers) {
            return res.status(StatusCodes.NOT_FOUND).json({ message: "No user at this time..." });
        }
...
userRouter.post("/register", async (req: Request, res: Response) => {
    try {
        ...
        if (user){
            return res.status(StatusCodes.BAD_REQUEST).json({ error: `This email has already beed registererd...`})
        }
...
userRouter.post("/login", async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(StatusCodes.NOT_FOUND).json({ error: `No user exist with the email provided..` });
        }
...
userRouter.put("/user/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (!username || !email || !password) {
            return res.status(401).json({error: `Please provide all the required parameter..`})     
        }
        if (!getUser) {
